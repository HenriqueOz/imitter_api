...
func Test_UpdateUserPassword(t *testing.T) {
	...
	t.Run("Should return a ErrUnexpected when fails to open transaction", func(tt *testing.T) {
		...
		newPassword := "new_randompassword"
		err := errors.New("cool error")

		mock.ExpectBegin().
			WillReturnError(err)

		result := repository.UpdateUserPassword(uuid, newPassword, password)
	...
	t.Run("Should return ErrUnexpected when fail to update user password", func(tt *testing.T) {
		...
		mock.ExpectQuery(`SELECT name FROM user WHERE uuid = \? AND password = \?`).
			...
			WillReturnRows(sqlmock.NewRows([]string{"name"}).
				AddRow("user"))
		mock.ExpectExec(`UPDATE user SET password = \? WHERE uuid = \?`).
			WithArgs(utils.HashSha256(newPassword), uuid).
